...
const Home: NextPage = () => {
    ...
    (async () => {
      try {
        if (session && session.accessToken && session.user  && session.user.email) {
          const fetchUser = router.query.userEmail || session.user.email          
          const { response, data } = await taskService.getUserTasks(String(session.accessToken), fetchUser.toString());
          setTasks(data.data)
        }
      } catch (error) {
        console.log(error);
  ...
  return (
    <>
      <Head>
        <title> My Tasks | UniKeeper </title>
